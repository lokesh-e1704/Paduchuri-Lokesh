...
void emptywait_north(){
    ...
    for(z = 0; z < xn; z++)
    {
        printf("North car %d gets released  \n",waitN[z]);
    }
    puts("");
    xn = 0;
}
void emptywaitS(){
    ...
    for(z = 0; z < xs; z++)
    {
        printf("South car %d gets released \n ",waitS[z]);
    }
    puts("");
    xs = 0;
}
...
{
    if(os_bridge.lk.lock == 0 || os_bridge.lk.lock == 2)
            {
...
{
    if(os_bridge.lk.lock == 0 || os_bridge.lk.lock == 1)
            {
...
int main()
{
    ...
    {
    	printf("Arrival Time of %d car : ",i);
